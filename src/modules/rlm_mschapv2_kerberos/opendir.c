				  ...
				  tDirNodeReference* userNodeRef, tDirReference dsRef)
{
	...
	if (!inUserName) {
		ERROR("rlm_mschapv2_kerberos: getUserNodeRef(): no username");
...
rlm_rcode_t od_mschap_auth(REQUEST *request, VALUE_PAIR *challenge, VALUE_PAIR * usernamepair)
{
	...
	*/
	...
	if (status != eDSNoErr) {
		...
		RERROR("rlm_mschapv2_kerberos: authentication failed - status = %s", status_name);
		free(status_name);
